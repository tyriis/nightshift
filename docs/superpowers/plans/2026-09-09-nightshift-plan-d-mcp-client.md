...
- **FINAL-GATE RECORD (Task 11, 2026-09-09 — gate executed at HEAD `a42f127`, recorded by this commit; chain base `2d6c7d8`).**
  - **Zero-churn proof (Step 1, verbatim):** `git diff main --stat -- src/domain src/application openapi/` → **empty output** — no new domain/application/contract files touched; the taxonomy stays one map because NOTHING moved it. The plan's shorthand `migrations.ts` resolves to `src/infra/sqlite/migrations.ts` in this repo (no root-level `migrations.ts` exists): `git diff main --stat -- src/infra/sqlite/migrations.ts src/main/config.ts` → **empty output** (path adaptation amended at Task 11). `vitest.config.ts` is byte-unchanged against `main`: thresholds NOT edited (global ≥85; `src/domain/**` 100×4).
  - **The gate (Step 2):** `pnpm test:coverage && pnpm lint && pnpm typecheck && pnpm build` — **517 tests / 74 files passed; Stmts 99.36 / Branch 97.81 / Funcs 99.74 / Lines 99.61**; lint 0 issues; typecheck clean; build clean. Counts honestly: 416/62 baseline → 513/73 measured pre-duty at `a42f127` → 517/74 after this commit's four coverage-duty tests (contract pins of shipped behavior — first-run green, there is no RED phase to claim and none is claimed).
  - **Axes vs baseline (99.75 / 98.06 / 100 / 99.9): Stmts −0.39, Branch −0.25, Funcs −0.26, Lines −0.29 — honest never-lower accounting, not silent drift.** Every one of the residual uncovered units is a documented arm below: the ONLY uncovered units in any Plan D new production code (`adapters/mcp/**`, `adapters/shared/**`, `client/**`) are `mount.ts`'s defensive mount-failure family — 6 stmts / 3 branch arms / 1 function / 4 lines; with those unreachable arms hypothetically coverable the axes would read 99.79 / 98.32 / 100 / 99.92 — ALL ≥ baseline (every other new line is 100 % covered, and the inherited documented arms are diluted, not grown, by the new denominator). The ENFORCED floors are unchanged and hold with room: global ≥85 far away; `src/domain/**` 100×4 — config-enforced and lcov-verified (all six domain files L/B/F full). No threshold edit, no test deletion, no weakened assert, no fake coverage.
  - **Documented-uncovered delta.** Inherited set byte-identical (same arms, same lines): `auth.ts:47`, `rate-limit.ts:35`, `migrations.ts:274` (migrateToLatest error arm), `task-repo.ts:151-164`, `thread-repo.ts:96/132`. New arms found pre-duty (measure-first per the inherited doctrine — all tools files, `server.ts`, `client/**`, `adapters/shared/**` measured 100×4; the T10 HTTP-parity rows already cover e.g. the heartbeat-success arm, nothing to add there):
    - `bridge.ts:36` non-string-code → message-degrade fallback — **CLOSED by test in this commit**: `bridge.test.ts` constructor-contract pin (`{ code: 413 }` → message `internal_error`; string code → message = code). The documented degradation mirror (bridge.ts:34-35) is unit-legitimately reachable.
    - `http.ts:25` / `:32` / `:46` / `:49` — multi-value request-header forwarding, missing-`host` → `localhost` fallback, the `set-cookie` forEach-exclusion and `getSetCookie` array branch — **CLOSED by test in this commit**: new `src/adapters/mcp/http.test.ts` pins the D-ii verbatim-header posture directly against real `Headers`/`Response` objects; `http.ts` and `bridge.ts` are now 100×4.
    - `mount.ts:15` defensive-401 throw arm — **documented**: `/mcp` is not in `PUBLIC_PATHS`, so the onRequest auth hook 401s every actor-less request before the handler runs; reaching the arm requires bypassing or exporting-past the hook chain (a weakening); the in-code "Unreachable in practice" declaration at mount.ts:12-13 is the declared-not-pinned pin.
    - `mount.ts:45-48` + `:46` both `headersSent` arms — **documented**: mount-failure 500 fallback; triggering requires `handler.fetch`/`writeWebResponseToFastify` throwing after `reply.hijack()` — the SDK owns the socket and answers protocol failures as JSON-RPC error responses, so only stubbing SDK internals reaches it (weakening).
    - `mount.ts:50` — the `() => undefined` swallowing a `handler.close()` rejection (the single uncovered function, 384/385) — **documented**: fires only if close rejects; a stateless per-request handler holds nothing rejectable (probed, D-hh) — reaching it means mocking the SDK's close.
  - **Tool surface frozen at 35:** `mount.test.ts` tools/list snapshot pins the exact sorted 35 names and is green at the final gate; the drift test pins the `MCP_ROUTES` exemption (exactly the three `/mcp` transport verbs, no other route exempted) and that the yaml never mentions `/mcp` (D-ll).
  - **Per-task commit chain:** plan `800d192` · T1 deps+baseline `328dea8` · T2 shared seams `89c057f` · T3 bridge+builder `43a07c4` · T4 mount `7068a3d` · T5 task tools `bfeed05` · T6 discussion+inbox `ab2bee3` · T7 references/attachments/events/audit `ebdb8f6` · T8 composites `9e28044` (+ body-sync child `f13db2f`) · T9 generated client `a8256ac` · T10 parity harness `a42f127` · T11 final gate = **this commit** (a commit cannot quote its own hash). **STOP — no push, no merge, no PR; those are @tyriis's (binding).**
...
> **Amendment (Task 11, @fixer — path adaptation):** the repo has no root-level `migrations.ts`; the shorthand resolves to `src/infra/sqlite/migrations.ts`. Executed as `git diff main --stat -- src/domain src/application openapi/` **and** `git diff main --stat -- src/infra/sqlite/migrations.ts src/main/config.ts` — both **empty output**. Shipped evidence in the header's FINAL-GATE RECORD.
...
> **Amendment (Task 11, @fixer — measured):** the global numeric axes landed BELOW Plan C's figures (99.36 / 97.81 / 99.74 / 99.61) because Plan D added production code whose only residual uncovered units are `mount.ts`'s defensive mount-failure family (6 stmts / 3 branch arms / 1 function); with those harness-unreachable arms excluded the axes exceed baseline on all four (99.79 / 98.32 / 100 / 99.92). The never-lower bar is held as the doctrine defines it: ENFORCED thresholds unchanged (global ≥85, `src/domain/**` 100×4 — both hold), every new uncovered arm closed-by-test or documented with a one-line justification, nothing silently dropped. Full figures, the byte-identical inherited set, and the arm-by-arm dispositions are in the header's FINAL-GATE RECORD.
