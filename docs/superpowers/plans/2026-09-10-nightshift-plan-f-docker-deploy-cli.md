...
- **FINAL-GATE RECORD (Task 8, 2026-09-10 — gate executed by the planner; this record is the final commit of Plan F; chain base `d8b7f7d` (PR #10 merge — Plans A–E merged).**
  - **The gate (planner-run, this record's evidence):** `pnpm test` → **674 passed / 88 files** (baseline 650/86); `pnpm test:coverage` → **99.52 / 98.52 / 99.78 / 99.70** — every axis ≥ the Plan E record (99.49 / 98.38 / 99.78 / 99.69): three axes MOVED UP, Funcs HELD at E's 99.78; `src/cli/run.ts` fully arm-covered per task (P10 held every commit; the lcov 0-uncovered verified at T2/T3). `lint` 0 errors · `typecheck` clean · `build` exit 0 · `ui:build` exit 0 · `ui:offline-check` → `ui build offline-safe; hosts seen: http://www.w3.org https://svelte.dev`. `pnpm test:e2e` PROBE-FIRST (D-xx posture: RUN) → `✓ §11.5 login -> board -> task -> comment -> status (800ms) / 1 passed` re-verified tonight. Honest residual: `format:check` still fails ONLY on the pre-existing spec md + `.slim/` scratch (unchanged since E; neither in any gate).
  - **Zero-churn proof (planner-run):** the full Task-8 Step-1 frozen-list diff against `main` is **EMPTY** — `vitest.config.ts`, `lefthook.yaml`, `ci.yaml`, `problem.ts`, `domain/errors.ts`, the 35-tool snapshot, `PUBLIC_PATHS`/`AUTH_PRE_SESSION` arms, `auth.ts`/`ui.ts`/`app.ts`, the whole `src/domain/**`+`src/application/**`+`src/infra/**`+`src/adapters/**`+`src/main/**` tree + `src/index.ts`, `openapi/`, `src/client/schema.d.ts`+`index.ts`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, the spec, all Plan A–E records — byte-untouched. **ZERO server change shipped: the D-aaa proof holds mechanically.** The sanctioned footprint is additive-only: `src/cli/` (run.ts + cli.test.ts + shim.test.ts), `src/testing/inject-fetch.ts`, `bin/nightshift.mjs`, `scripts/deploy-smoke.mjs`+`scripts/fts-probe.mjs`, `deploy/README.md`+`deploy/DEPLOY-SMOKE.md`, `Dockerfile`, `.dockerignore`, package.json's `bin`+`cli` two keys (+4), this doc.
  - **The CLI chain D-oo→D-yy→D-aaa is CLOSED:** `nightshift next|claim|report` ships over the drift-pinned D-kk client, zero new runtime deps, zero new taxonomy codes (three CLI-side stderr strings live stderr-only, never the contract), the pinned ladder `0 ok · 1 transport_error · 2 usage/config · 3 API problem` with machine strings byte-exact + README-mirrored, `kind: question` deliberately unexposed, lease via env never argv (D-ff), the five-line shim the only `process`/`globalThis.fetch` site (D-bbb), spawn-pinned + in-image-proven.
  - **The image (D-ccc/D-ddd, fresh `docker build --no-cache` at this gate):** new id `bf00fe3f0fad` (≠ ship-time `7752986e3888`) proves the no-cache rebuild is genuine; BUILDER FORM: docker 29.7.2 LEGACY builder (no buildx — S4 recorded; Dockerfile builder-agnostic). The offline guard ran INSIDE the image build — `ui build offline-safe; hosts seen: http://www.w3.org https://svelte.dev` quoted verbatim from the build log. Config pins: `User node` (uid 1000 live), `ENTRYPOINT node dist/index.js`, `ENV NS_DB_PATH=/data/nightshift.db NS_DATA_DIR=/data`, HEALTHCHECK `node -e` both arms live-checked (`--start-interval=1s` accepted, not dropped), `ExposedPorts 3123`, `Volumes == null` (explicit-mount posture), `docker history` clean of toolchain in runtime layers, zero secret material in any layer. The D-ccc cwd story PROVEN byte-identity: served `/openapi.yaml` `diff`-identical to the checkout; `/ui` shell + SPA deep-link from WORKDIR `/app`; `/data` = db + WAL sidecars while running; `/app` read-only for the runtime user. **Graceful stop: `docker stop -t 12` → ExitCode 0** (measured ship-time AND again at this fresh-boot gate — the signal-handling claim is measured, not claimed).
  - **Deploy evidence (D-eee — what RAN here vs what the operator runs at home):** `scripts/deploy-smoke.mjs` ran against the ship-time container AND the rebuilt image: **22 PASS / 1 recorded-FAIL both times.** The one FAIL is a REAL SERVER FINDING this plan surfaced, not script drift: under `Accept-Encoding: br` the preCompressed `/ui` shell variant bypasses `ui.ts`'s `.html` `setHeaders` check and answers `public, max-age=2592000, immutable` instead of no-cache (a 30 d stale-shell hazard; inject-pinned Plan-E arms cannot see encoding negotiation — the deploy-smoke gate caught what unit tests structurally could not). F's zero-server-change ruling is BINDING, so the fix (`ui.ts` setHeaders + its Plan-E test arms) is an OWNER follow-up — named in the PR body, probe never weakened, the gate bar amended BEFORE the final run (`d95dab4`), never smuggled. `scripts/fts-probe.mjs`: both declared-not-pinned FTS5 arms (Plan C lineage) **PASS on a stopped-container volume COPY** — MATCH data-carry + delete-trigger prune; the two execution corrections (FTS5 bare-token column-exclusion grammar; better-sqlite3 FKs measure ON) are Amendment-logged, arms executable-not-weakened. The §14 walkthrough's agent-side spine is automated-green; the human/browser legs (real Pocket ID client registration, the `INTERNAL_APP_URL` reachability check, **nonce-echo verify-once** checkbox, allow-list round-trip, two-humans close) are OPERATOR-side per `deploy/DEPLOY-SMOKE.md` 0–8 with copy-pasteable commands + the residue ledger (one canceled `deploy-smoke` task + note + attachment + revoked token + audit spine — by design, audit-visible).
  - **Process record:** pre-dispatch = review attempt-1 FAIL (B1–B6 all applied as logged amendments) + preflight P1–P10 ALL PASS (live-verified server arms incl. `stale_lease`-412 and the race flat-envelope; the plan's own FTS5-grammar + FK assumptions caught and corrected). Every task = fresh implementer + independent two-stage (spec THEN quality) review with PASS before the next dispatch; per-task divergences carry byte-sync lineage (T2 eslint-typed-no-op cast, T3 prettier re-flows + the coverage-forcing ghost arm "prove it, not document it" per Plan D precedent, T4 prettier bin-map, T6 script truths); post-amendment review attempts n-covered (n7–n13, n26–n32 dispositions). A machine crash mid-Task-7: the orphaned README was kept ONLY after the independent review lane diffed every table row / VERBATIM quote / behavior claim against the code (STAGE1+STAGE2 PASS; the parent-flagged `review_gate` clause resolved in the doc's favor — `update-status.ts:72-79`). **Raw-git verification: every hash below re-checked against `git log origin/main..HEAD` at record time — no phantom commits.**
  - **The commit chain:** plan `86fb9d8` · review+preflight amendments `0d46914` · review notes n7/n8 `2124ed5` · n13 pre-T3 fix `3de7615` · T1 `6afd196` · T2 `45330b4` · T3 `52547ce` · T4 `9a34e92` · T5 `425f456` · T6 `3e70c72` · finding gate-amendment `d95dab4` · T7 `f348b33` · n29 `374523b` · this record.
  - **Byte-sync audit:** every shipped divergence carries its amendment + `sync = shipped form` re-label; the plan-failure scan (`TBD|TODO|fill in|similar to Task`) matches ONLY its own scan-command line (E's paradox, expected — zero plan-failure text).
  - **QUEUED (HUMAN) — unchanged, nothing blocked tonight, each routed around + named:** real Pocket ID client credentials + `INTERNAL_APP_URL` caveat + nonce verify-once (D-pp — checklist §4) · Playwright browser re-install after upgrades + optional ubuntu-runner CI e2e job (`ci.yaml` byte-untouched; D-xx) · nonce/hash-grade CSP (§12 candidate) · session-key dual-rotation (D-qq) · `removeAdditional` 400-flip · `nightshift-client` npm publish + extensionless `bin/nightshift` exec form · home-ops HelmRelease + registry push + digest pin. **NEW from tonight: the brotli shell-cache server finding — owner decision on the follow-up fix.**
  - **Slice-outs honored (state frozen):** SSE, search UI / saved filters / undo, keepalive enforcement, capability scopes, §13 items — zero touched.
  - **End of night per the ticket #11 protocol: gates green ⇒ branch pushed + PR opened (`resolves #11`), redaction pass applied (no local absolute paths, usernames, or hostnames; the local smoke literal is the documented plan value). NEVER merge — merges are @tyriis's (binding).** After Plan F the §12 surface is complete — what remains is the §12 backlog + the brotli follow-up + whatever the first real-world week shakes loose.
...
- [ ] **Step 2: The gate:** `pnpm test:coverage && pnpm lint && pnpm typecheck && pnpm build && pnpm ui:build && pnpm ui:offline-check && pnpm test:e2e` — every global axis ≥ the Plan E record (**99.49 / 98.38 / 99.78 / 99.69**, never-lower HELD — the `src/cli/**` arms closed by test, P10; B6 fix: E's Funcs axis is 99.78); `test:e2e` probe-first per D-xx (browser may still be installed from Plan E — honest posture either way); THEN the F-native gates re-run for the record: `docker build` (fresh, no cache — the offline gate line quoted verbatim) → boot → `node scripts/deploy-smoke.mjs` → `ALL PASS` + the graceful `docker stop` exit-0 measurement. Honest counts recorded (650 → N tests / 86 → N files). (Task 6 EXECUTION FINDING — gate expectation amended BEFORE Task 8, with lineage: the live run is **22 PASS / 1 recorded-FAIL**. The FAIL is a SERVER finding, not a script defect: under `Accept-Encoding: br` negotiation the preCompressed shell variant bypasses `ui.ts`'s `setHeaders` `.html` check and answers `public, max-age=2592000, immutable` instead of no-cache — the plugin default hits the `.br` path; inject-pinned E arms cannot see encoding negotiation, the live deploy-smoke caught it (the E "browser catches what inject can't" lineage). Plan F ships ZERO server changes (binding) — the gate bar is ALL-PASS EXCEPT that one recorded arm; the fix (a `ui.ts` setHeaders re-pin — Plan E surface, E test arms included) is an owner follow-up decision, surfaced in the PR body and the FINAL-GATE RECORD, never smuggled into F.)
