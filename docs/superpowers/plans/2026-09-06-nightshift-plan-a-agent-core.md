...
> **Amendment (Task 17 hardening, reviewer exp-15):** the pretty tree can carry node lines the
> regex cannot parse — a wildcard route prints as a bare `── * (GET, HEAD)` leaf with no path
> segment, and exp-15 proved the silent skip read as false GREEN. Shipped parser now throws on any
> `── ` line it cannot parse (wildcard / method-constraint / find-my-way-format-change classes all
