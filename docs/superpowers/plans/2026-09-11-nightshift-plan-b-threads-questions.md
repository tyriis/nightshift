...
> **Amendment (Task 7, self-check §7.3 replay pin — fold-in round):** The self-check `§7.3` row's promise "verify in Task 7 test that one POST with `Idempotency-Key` replays" now has a test. `routes/threads.test.ts` gained `it('one POST with an Idempotency-Key replays the stored 201 (self-check §7.3, inherited hook)')`: one `POST /tasks/:id/threads` carrying an `Idempotency-Key`, then the same POST repeated, pinning the replay's stored `201`, a byte-identical body (`replay.body` === `first.body`), the same `thread.id`, and `GET /tasks/:id/threads` length unchanged — the replay wrote nothing. This is a **VERIFICATION pin of inherited app-level behavior, not new behavior**: the `onRequest`/`onSend` pair in `idempotency.ts` already covers every authenticated mutating route by construction, so the pin PASSED on its first run — there is no RED phase to claim and none is claimed; it would have gone RED only if the composition had broken, and had it, the fold-in would have stopped as an integration defect rather than shipping. It supersedes the Task 16 amendment's observation that the row "never shipped in `threads.test.ts`" (now stale by design, noted there); the self-check table itself stays untouched — the row's parenthetical records where the verification belongs, and the verification now exists. Block sync = shipped form.
...
> **Amendment (Task 15, story shipped verbatim — passed first-run):** `scenarios-discussion.test.ts` shipped as the block wrote it, save one delta: the block's `void anaId` line was dropped (`anaId` is used at the token URL, so the `void` pre-empted nothing — removal semantics-neutral). Its one open question (does the shipped `/audit` route accept `entity_id` without `entity_type`, with `limit`?) was checked against `routes/audit.ts` first: both filters are optional and `limit` defaults to 50, so the story's `?entity_id=…&limit=50` works as written and the block's "amend if both are required" branch did not fire. Per Step 15.2 this task cannot be RED in the TDD sense; it PASSED on the first run (nothing to fix on either side — the story does not misread the shipped surface, and no subsystem defect was exposed). Machine-string note: the story drives the `in_review` transition that auto-releases the claim, but it does not assert `claim released on review` — that byte-exact string is already pinned by `update-status.test.ts` and the Task 8 REST pin, so no duplication here; the story's job is that invariant 6 fires and lifts over HTTP exactly once, and it does. Block sync = shipped form.

> **Amendment (Task 15, quality review — self-naming pins, Minor-only):** Three touches to `scenarios-discussion.test.ts`; the story's arc, actors, and ordering are untouched. (a) The two bare injects — the note-thread `POST /tasks/:id/threads` and the resolved-state `PATCH /tasks/:id/threads/:tid` — are now named and status-pinned (`expect(note.statusCode).toBe(201)`, `expect(resolved.statusCode).toBe(200)`), so a step that fails names itself instead of surfacing as a downstream `audit` array mismatch. (b) The gated-409 pin grew from `code` alone to the T12-verified flat envelope — `expect(blocked.json()).toMatchObject({ code: 'open_questions', open: 1 })` — carrying into the acceptance story the wire shape the Task 8 REST pin in `routes/threads.test.ts` already asserts (`problem.ts` spreads details top-level beside `code`). DECLINED (the review's optional context-bundle-payoff suggestion): the story shipped block-verbatim and that payoff is already pinned twice over — by Task 10's `queries.test.ts` (`bundle.attachments[0]` on the populated case) and by this story's own mid-arc `context.attachments … toContain('spec.md')` — so a third pin is duplication, not coverage. All three additions live inside the single existing `it`, so the test count does not move and no behavior was touched. Block sync = shipped form.
...
> **Amendment (Task 16, router strictness + consumed items + one self-check wording miss):** The three steps shipped as written — `auth.ts` carries the block's EXACT normalization line (prettier-collapsed to one line), the sanctioned-import note (matching manage-actors' ruling style), `ci.yaml` swaps `pnpm test` → `pnpm test:coverage` (CI-side effect untestable locally; step style mirrored 1:1). Two probe-backed corrections to Step 16.1's expectations: the shipped router runs with `ignoreTrailingSlash` unset, so slash variants of `/ping` PASS AUTH (the normalization's actual effect — no more 401) but the ROUTER answers 404, not the block's `→ 200`; likewise `GET /audit/` WITH a token is 404, not the block's `200 with`. Probe evidence: `/ping -> 200`, `/ping/ -> 404`, `/ping// -> 404`, `//ping -> 404`, `authed /audit/ -> 404`. The pin asserts the fail-closed truth (public variants never hit the auth wall; protected variants ALWAYS do), and widening routing itself (`ignoreTrailingSlash`) is deliberately OUT — the block scoped this to the lookup. The block's `?? '/'` on `split('?')[0]` is an unreachable defensive arm (split always yields index 0) — kept verbatim, it is `auth.ts`'s only uncovered branch (92.85% branch, 100% lines). Consumed/skipped backlog items: the `problem.ts` 413/415 mapping shipped in Task 2 (drift-pinned) and appears nowhere in this block's steps — noted, not redone; the Docker/Plan-F `openapi/` asset-shipping note is NOT in this block (it stays where the plan's deferred list already owns it — Plan F), and nothing was invented for it. Self-check cross-check: the `§7.3` row's "verify in Task 7 test that one POST with Idempotency-Key replays" never shipped in `threads.test.ts` — replay semantics are pinned at the middleware (`idempotency.test.ts`) and by the 204-replay pin in `deps-labels.test.ts`, and the app-level hook covers the new POST routes by construction; reported as a wording miss, the self-check table left untouched per instruction. Final gate: all green (373 tests; Stmts 99.71 / Branch 97.91 / Funcs 100 / Lines 99.89, domain 100; lint/tsc/build exit 0). Block sync = shipped form.

> **Amendment (Task 16, honesty pass — no rename needed):** The fold-in round asked to rename the Task 16 test if its name promised a `GET /ping/ → 200` the assertions did not pin. Checked against the shipped file and `git log`: the name at HEAD is `it('public-path lookup tolerates slash spellings while protected paths stay fail-closed (Task 16)')` — it promises auth-wall passage and fail-closure, which are exactly what the assertions pin ([200, 404] never 401 for public spellings; 401 for protected ones; authed `/audit/` 404). No committed name ever claimed a 200 for the slash variants (the `200 (was 401)` wording lives in Step 16.1's block text, which this amendment already corrects). NO-OP: name and assertions already agree, so nothing was renamed and behavior stays untouched. Cross-note: the §7.3 wording miss recorded in the amendment immediately above is now superseded — the replay pin shipped in `routes/threads.test.ts` in the same fold-in round (see the Task 7 amendment); the self-check table was again left untouched. Block sync = shipped form.
