...
- [ ] **Step 3b: Two sanctioned doc-comment lines (D-hhh quality advisories #3/#7, carried per ledger).** In `src/application/ports.ts` on `listStaleClaims` append: ``limit` must be >= 0 — SQLite treats a negative LIMIT as unlimited (the sweeper passes BATCH; advisory #7).`and on the coalesce clause of the same comment append:`Legacy-row caveat: a never-heartbeating PRE-G claim anchors on updated_at — frequent content edits keep it alive; accept/backfill is an operator question (advisory #3).`In`src/infra/sqlite/task-repo.ts`the`listStaleClaims` body comment gains a matching one-line pointer. Comment-only; zero logic.
...
- **Task 3 COMPLETE — `1f7b13f`.** Honest RED BOTH faces (vitest 3-fail verbatim + the sanctioned 8-error TS face) → GREEN task-repo 16/16, full `pnpm test` 683/88, lint/typecheck clean, 3-file diff (+153/−2) raw-git-verified. Claim/model arms stayed GREEN — the D-hhh stamp is harmless exactly as Step 6 predicted (escalation path unused). A2 confirmed: fakes widened NONE (cast seams held). SPEC-REVIEW: PASS (coalesce/canceled/CAS verified live; both cutoffs carry the P4 01-05 amendment; cancel-arm remains discriminating). QUALITY: PASS — advisories dispositioned: **#3 legacy-coalesce caveat + #7 negative-`limit` trust contract: SANCTIONED as two doc-comment lines carried into Task 4's diff** (the sweeper is the primitives' first consumer — the trust contract belongs with its author; lineage "D-hhh quality advisories #3/#7" in that commit body); remaining points: no change (Kysely bind-safety verified, file doctrine holds). Zero implementer drift.
- **Task 4 COMPLETE — `a09e0d3`.** Honest RED (module-missing compile face, verbatim) → GREEN sweeper 8/8 (all P10 arms by name; repeat-run zero flake), full `pnpm test` **691/89 — exactly P9's predicted integration count**, lint/typecheck clean, 7-file diff (+319/−3) raw-git-verified, heartbeat.ts confirmed comment-only. SPEC-REVIEW: PASS (wiring ordering source-verified; Step 3b sentences verbatim; the `!` carries its P6 license in-comment). QUALITY: PASS — delivery-loop parity verified line-by-line (the UoW-divergence comment explains WHY at both reading sites); per-arm isolation, honest casts, drain-before-close structurally closed. Deviations dispositioned: (a) deps-comment `D-iii:` prefix — sanctioned-shape addition, keep; (b) task-repo pointer wording — comment-only, consolidates advisories #3/#7 in one doc home, keep; **(c) commit-subject drift (`+ dormant wiring` vs plan's `, dormant by default`) — REAL but cosmetic; the "ticket superseded" claim is ungrounded (ticket pins no subject); ledger-note per review lane, NO history rewrite.** Advisories dispositioned: A1 ("ONE stated divergence" phrasing — every divergence IS documented at its site) and A2 (test-header dormant-default pointer) — **recorded, no re-run** (reviewer: "not worth one"). Zero logic drift.
