...
- **FINAL gate (execution, 2026-09-09, post-Task-16 @ `b42dbf2`): 416 tests / 62 files; Stmts 99.75 / Branch 98.06 / Funcs 100 / Lines 99.9** — never-lower HELD (every axis equal or above both baseline figures; net +42 tests). lint / typecheck / build clean. The uncovered set is still exactly the documented arms (`auth.ts:47`, `rate-limit.ts:35`, `task-repo.ts:151-164`, `thread-repo.ts:96/132`; `migrations.ts:193` → `:274` after insertions — same `migrateToLatest` error arm) — **no new uncovered arms**. Declared-not-pinned (per protocol, no fake RED): the FTS in-migration `rebuild` DATA-CARRY arm (no-op on fresh CI databases; fires on homelab upgrade — Task 13 text) and the FTS delete-trigger arm (no task-delete path exists — cancel is terminal). Task 11's amendment shipped the loop's `start()` interval-body + dead-pool-fire tests, so the originally-anticipated `start()`-body addition did not materialize.
- Follow-up for @tyriis (review-ruled, deliberately NOT silently applied): `routes/admin.ts` now carries the type+value two-import pattern the links.ts backlog nit merged against (the Task 16 plan block prescribes it verbatim; byte-sync doctrine forbade deviating). One-line symmetry fix when convenient: `import { ACTOR_KINDS, type ActorKind } from '#root/domain/task'` — zero behavior.
...
> **Amendment (Task 10, explicit-return-type repair on `toEvent`):** Three byte-sync notes. (1) The Step 3 block does not pass the repo's lint as planned: `pnpm lint` errors `events.ts 8:31 Missing return type on function` (`@typescript-eslint/explicit-function-return-type`; `allowExpressions` covers expression-position functions only, not variable-initialized arrows). Repaired with a local `interface FeedEvent` (the eight wire fields, nullables as `string | null`; the route-module local-interface precedent is `tasks.ts`'s `interface CreateTaskBody`) and `const toEvent = (r: AuditRow): FeedEvent => ({…})` — the mapping body, both comments and everything else in the file are byte-identical to the block. The Step 1 test block is byte-identical to disk (diffed against the plan's own lines — zero diff; `prettier --check` clean as written). (2) The Step 4 yaml blocks land embedded: paths at the file's `paths:` nesting indent (+2), the `Event` schema at `components.schemas` (+4), and the blocks' single-quoted `'200'`/`'#/components/…'`/`['string', 'null']` tokens ship in the file's double-quote style — Task 2/9 precedent, quote characters only, structure as planned (including the `limit` parameter's prettier-hugged flow map). `Event.description` stays the block's single long line — prettier preserves long plain yaml scalars (disk precedent: the `listWebhooks` description). Normalizations verified mechanically: block + embed-indent + quote-swap diffs zero against disk. (3) The app.ts registration follows the step's placement prose (after `registerAuditRoutes`) with a house comment (`// the cursor feed reads the same spine (D-aa)`, Task 9 precedent). Honest RED: Step 2 failed as anticipated — `Tests 4 failed | 1 passed (5)` — with `AssertionError: expected { …(5) } to deeply equal []` (the `{ …(5) }` is the problem body, `"code": "not_found"`), `cursor=-1: expected 404 to be 400`, `expected 404 to be 200` (the D-dd pin's first request), `expected { …(5) } to have property 'length'` — the 404 is problem.ts's notFoundHandler `not_found`. The 401 auth pin passed pre-implementation because the auth `onRequest` hook answers before route matching — stated honestly; no fake RED claimed. Block sync = shipped form.
